...
def train_target(args):
    ...
    modelpath = args.output_dir_src + '/' + str(args.s) + '_F.pt'
    netF.load_state_dict(torch.load(modelpath))
    modelpath = args.output_dir_src + '/' + str(args.s) + '_B.pt'
    netB.load_state_dict(torch.load(modelpath))
    modelpath = args.output_dir_src + '/' + str(args.s) + '_C.pt'
    ...
    if args.issave:
        torch.save(netF.state_dict(), osp.join(args.output_dir, str(args.t) + "_F.pt"))#CL
        torch.save(netB.state_dict(), osp.join(args.output_dir, str(args.t) + "_B.pt"))#CL
        torch.save(netC.state_dict(), osp.join(args.output_dir, str(args.t) + "_C.pt"))#CL
...
def test_target(args, dset_loaders, domain_num, cur_dom):
    ## set base network
    netF = network.ResBase(res_name=args.net).cuda()
    netB = network.feat_bootleneck(type=args.classifier, feature_dim=netF.in_features, bottleneck_dim=args.bottleneck).cuda()
    netC = network.feat_classifier(type=args.layer, class_num = args.class_num, bottleneck_dim=args.bottleneck).cuda()
    
    args.modelpath = args.model_dir + '/' + str(cur_dom)+ '_F.pt'
    netF.load_state_dict(torch.load(args.modelpath))
    args.modelpath = args.model_dir + '/' + str(cur_dom)+ '_B.pt'
    netB.load_state_dict(torch.load(args.modelpath))
    args.modelpath = args.model_dir + '/' + str(cur_dom)+ '_C.pt'
    ...
    log_str = 'Task: {}, Accuracy = {:.2f}%'.format(args.name, acc)
...
if __name__ == "__main__":
    ...
    args.output_dir_src = osp.join(args.output_src, str(args.s)+names[0][0].upper())
    ...
    args.output_dir = osp.join(args.output, str(args.t)+names[0][0].upper())#CL
    args.name = str(args.t)+names[0][0].upper()#CL
    ...
    args.out_file = open(osp.join(args.output_dir, 'log_test.txt'), 'w')
    cur_dom = args.t
    for i in range(len(names)):

        args.t = i
        args.name = names[0][0].upper() + names[args.t][0].upper()

        args.s_dset_path = folder + args.dset + '/' + names[args.s] + '_list.txt'
        args.test_dset_path = folder + args.dset + '/' + names[args.t] + '_list.txt'

        if i == 0:
            dset_loaders = dset_loaders_source
        else:
            dset_loaders = data_load(args)
        args.model_dir = osp.join(args.output, str(cur_dom)+names[0][0].upper())

        test_target(args, dset_loaders, i, cur_dom)
