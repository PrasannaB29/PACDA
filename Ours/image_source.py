...
def train_source(args, dset_loaders):
    ...
    while iter_num < max_iter:
        ...
        print("Iter {} done".format(iter_num))
        netF.train()
        netB.train()
        netC.train()

    # acc_list_array = np.asarray(acc_list)
    # np.save(osp.join(args.output_dir_src, "acc_list.npy"))    
    torch.save(best_netF, osp.join(args.output_dir_src, "0_F.pt"))
    torch.save(best_netB, osp.join(args.output_dir_src, "0_B.pt"))
    torch.save(best_netC, osp.join(args.output_dir_src, "0_C.pt"))
...
def test_target(args, dset_loaders, domain_num):
    ...
    args.modelpath = args.output_dir_src + '/0_F.pt'
    netF.load_state_dict(torch.load(args.modelpath))
    args.modelpath = args.output_dir_src + '/0_B.pt'
    netB.load_state_dict(torch.load(args.modelpath))
    args.modelpath = args.output_dir_src + '/0_C.pt'
    ...
    acc, _ = cal_acc(loader, netF, netB, netC, False)
...
if __name__ == "__main__":
    ...
    train_source(args, dset_loaders_source)
